...
2) Ask LLM (watsonx.ai) to decide if web search is needed:
...
# ---------------------
# LLM (watsonx.ai native chat) — replaces openai_chat
# ---------------------
def watsonx_chat(
    base_url: str,
    api_key: str,
...
def llm_decide_search(
    base_url: str, api_key: str, model: str, project_id: str,
    question: str, transcript_context: str, explicit_flag: bool
) -> Dict:
    """
    Ask watsonx.ai to decide whether to search the web.
    ...
    out = watsonx_chat(
        base_url=base_url, api_key=api_key, model=model, project_id=project_id,
...
def on_chat(
    ...
    base_url, api_key, model, project_id, embed_device, embed_model_override,
    ...
    if not video_path:
        ...
        return
    if not base_url or not api_key or not model or not project_id:
        msgs.append({"role": "assistant", "content": "Please set watsonx Base URL, API key, Model and Project ID in the panel."})
        yield msgs, gr.update(choices=[], value=None, visible=False), {}
        return
    ...
    # 4) Final LLM call with transcript + web context (watsonx)
    system = (
...
# Gradio Layout
# ---------------------
with gr.Blocks(title="Video RAG Chat (Gradio + watsonx.ai + Exa)", fill_height=True) as demo:
    ...
    send_btn.click(
        ...
        inputs=[
            ...
            base_url_tb, api_key_tb, model_tb, project_tb, embed_device_q_dd, embed_model_override_tb,

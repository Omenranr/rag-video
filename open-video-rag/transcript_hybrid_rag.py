...
class TranscriptRAGIndex:
    ...
    def build_from_srt(cls, srt_path: Path, outdir: Optional[Path] = None, window_sec: int = 5,
        ...
        print(f"[build] Parsing SRT: {srt_path}")
        entries = parse_srt(srt_path)

        print(f"[build] Windowing into {window_sec}s chunks (anchor={anchor})")
        ...
        meta = Meta(model_name=model_name, window_sec=window_sec, built_at=now_stamp(),
                    ...
                    sources=("transcript",), extra_files={})
        return cls._finalize_build(df, outdir, model_name, device, meta)

    @classmethod
    def build_from_dataframe(cls, df: pd.DataFrame, outdir: Path,
                             model_name: str = "sentence-transformers/all-MiniLM-L6-v2",
                             device: Optional[str] = None,
                             meta_overrides: Optional[Dict[str, Any]] = None) -> "TranscriptRAGIndex":
        outdir = Path(outdir).resolve()
        outdir.mkdir(parents=True, exist_ok=True)
        overrides = meta_overrides or {}
        extra_files = overrides.get("extra_files", {}) or {}
        extra_files = {str(k): str(v) for k, v in extra_files.items()}
        meta = Meta(
            model_name=model_name,
            window_sec=int(overrides.get("window_sec", 0)),
            built_at=now_stamp(),
            num_chunks=len(df) if df is not None else 0,
            srt_source=str(overrides.get("srt_source", "")),
            anchor=str(overrides.get("anchor", "custom")),
            sources=tuple(overrides.get("sources", ("transcript",))),
            extra_files=extra_files
        )
        return cls._finalize_build(df, outdir, model_name, device, meta)

    @classmethod
    def _finalize_build(cls, df: pd.DataFrame, outdir: Path,
                        model_name: str, device: Optional[str],
                        meta: Meta) -> "TranscriptRAGIndex":
        if df is None or df.empty:
            raise RuntimeError("No chunks provided for index build.")

        required_cols = {"chunk_id", "start", "end", "start_srt", "end_srt", "text"}
        missing = required_cols - set(df.columns)
        if missing:
            raise ValueError(f"Missing required columns for index build: {sorted(missing)}")

        cleaned = df.copy()
        cleaned["text"] = cleaned["text"].fillna("").astype(str)
        cleaned["start"] = cleaned["start"].astype(float)
        cleaned["end"] = cleaned["end"].astype(float)
        cleaned["chunk_id"] = cleaned["chunk_id"].astype(str)
        cleaned["start_srt"] = cleaned["start_srt"].astype(str)
        cleaned["end_srt"] = cleaned["end_srt"].astype(str)
        if "source" not in cleaned.columns:
            cleaned["source"] = "transcript"

        cleaned = cleaned.sort_values("start").reset_index(drop=True)
        if cleaned["chunk_id"].duplicated().any():
            dupes = cleaned.loc[cleaned["chunk_id"].duplicated(), "chunk_id"].tolist()
            raise ValueError(f"Duplicate chunk_id(s) encountered: {dupes[:5]}")

        outdir.mkdir(parents=True, exist_ok=True)
        cleaned.to_json(outdir / "chunks.jsonl", orient="records", lines=True, force_ascii=False)

        print("[build] Building BM25 index (FR-aware)")
        corpus_tokens = [simple_tokenize_fr(t) for t in cleaned["text"].tolist()]
    ...
    def context_window_for_index(self, center_idx: int, before: int, after: int) -> List[Dict]:
        ...
        for j in range(lo, hi + 1):
            ...
            for extra_key in ("source", "scene_id", "vlm_kind"):
                if extra_key in r.index:

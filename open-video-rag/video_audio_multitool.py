...
def run_pipeline(
    ...
    # Make transcript alias predictable (do this BEFORE detection so it's always present)
    ...
    do_detections = (not no_detect) and (local_video_path is not None) and (int(det_max) != 0)
    if not do_detections:
        logger.info("Skipping visual detections (no_detect=%s, has_video=%s, max_det=%s).",
                    no_detect, local_video_path is not None, det_max)
        # Write empty CSVs with headers so downstream code is happy
        det_csv = out_dir / "detections.csv"
        by_frame_csv = out_dir / "detections_by_frame.csv"

        pd.DataFrame(columns=[
            "frame_index","time","time_seconds","label","conf","x1","y1","x2","y2","extra"
...
def main():
    ...
    parser.add_argument("--no-detect", action="store_true",
                    help="Skip visual detections entirely (only transcribe).")
